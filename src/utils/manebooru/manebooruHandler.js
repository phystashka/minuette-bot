...
export const handleManebooruPagination = async (interaction) => {
  try {
    ...
    console.error('Error in Manebooru pagination:', error);
...
export const fetchImages = async (query, limit = 1, filterOption = 'DEFAULT', page = 1) => {
  try {
    const baseUrl = 'https://manebooru.art/api/v1/json/search/images';
    
    let searchQuery = query && query.trim() ? query.trim() : '*';
    
    // Всегда добавляем safe фильтр
    if (searchQuery === '*') {
      searchQuery = 'safe';
    } else {
      searchQuery += ',safe';
    ...
    console.error('Error fetching Manebooru images:', error);
    return [];
  }
};
...
export const displayImage = async (interaction, images, currentIndex, query, filterOption) => {
  try {
    ...
    if (!image) {
      return await interaction.editReply({
        embeds: [
          createEmbed({
            title: "Error",
            description: "Received invalid data from Manebooru API.",
            color: 0xED4245
          })
        ],
        components: []
    ...
    const prevButton = createButton({
      customId: `manebooru_prev_${image.id}_${encodeURIComponent(shortQuery)}_${filterId}_${currentIndex}`,
    ...
    const nextButton = createButton({
      customId: `manebooru_next_${image.id}_${encodeURIComponent(shortQuery)}_${filterId}_${currentIndex}`,
    ...
    const sourceButton = createButton({
      label: 'View on Manebooru',
      url: `https://manebooru.art/images/${image.id}`,
      emoji: '🔍'
    });
    ...
    console.error('Error displaying Manebooru image:', error);
    
    const errorEmbed = createEmbed({
      title: "Oopsie!",
      description: `I had trouble displaying the image from Manebooru. Maybe try again later?\n\nError: ${error.message}`,
      color: 0xED4245
    });
