...
import { FILTERS, handleDanbooruPagination, displayImage, fetchImages, processQuery, createImageEmbed } from '../../utils/danbooru/danbooruHandler.js';
...
export async function execute(interaction) {
  ...
  try {
    ...
    return interaction.editReply({ 
      embeds: [createEmbed({
        title: "Error",
        description: `An error occurred while searching for art on Danbooru: ${error.message}`,
        color: 0xED4245
      })] 
    });
  }
...
async function displayMultipleImages(interaction, images, count, query, filterOption) {
  try {
    ...
    const firstSourceButton = createButton({
      ...
      url: `https://danbooru.donmai.us/posts/${firstImage.id}`,
      emoji: '🔍'
    });
    ...
    for (let i = 1; i < images.length && i < count; i++) {
      ...
      const sourceButton = createButton({
        label: 'View on Danbooru',
        url: `https://danbooru.donmai.us/posts/${images[i].id}`,
        emoji: '🔍'
      });
