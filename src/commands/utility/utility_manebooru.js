...
import { FILTERS, handleManebooruPagination, displayImage, fetchImages, processQuery, createImageEmbed } from '../../utils/manebooru/manebooruHandler.js';
...
export async function execute(interaction) {
  ...
  try {
    ...
    return interaction.editReply({ 
      embeds: [createEmbed({
        title: "Error",
        description: `An error occurred while searching for art on Manebooru: ${error.message}`,
        color: 0xED4245
      })] 
    });
  }
...
async function displayMultipleImages(interaction, images, count, query, filterOption) {
  try {
    ...
    const firstSourceButton = createButton({
      ...
      url: `https://manebooru.art/images/${firstImage.id}`,
      emoji: '🔍'
    });
    ...
    for (let i = 1; i < images.length && i < count; i++) {
      ...
      const sourceButton = createButton({
        label: 'View on Manebooru',
        url: `https://manebooru.art/images/${images[i].id}`,
        emoji: '🔍'
      });
